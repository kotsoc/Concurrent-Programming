...
%initial state
initial_state() ->
    #server_st{uselist = [],nicklist =[], channelL = []}.
...
%% Change Nickname
listen(St, {nick,OldNick, Nickname}) ->
    Exist = lists:member(Nickname, St#server_st.nicklist),
    %% check if nickname is in the list
    if Exist =:= false ->
        Nck = [Nickname|St#server_st.nicklist],
        lists:delete(OldNick, Nck),
        NewSt = St#server_st{nicklist = Nck},
        Resp = ok;
        
    true ->
        NewSt = St,
        Resp = {error, nick_taken, "nick_is_taken"}
    end,
    
{reply, Resp, NewSt};

%% Join channel
listen(St, {join, Pid, Nickname, Channel}) ->
    Exist = lists:member(Channel, St#server_st.channelL),
    ExistNick = lists:member(Nickname, St#server_st.nicklist),
    
    if Exist =:= false ->
        if ExistNick =:= false ->
            Nck = [Nickname|St#server_st.nicklist];
        true ->
            Nck = St#server_st.nicklist
        end,
        
        Cha = [Channel|St#server_st.channelL],
        NewSt = St#server_st{channelL = Cha, nicklist = Nck},
